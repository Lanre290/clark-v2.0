...
interface ConfigInterface {
  ...
  responseSchema: {
    type: string;
    properties: {
      answer: { type: string; description: string };
      chat_title?: { type: string; description: string };
    };
    required: string[];
  };
}
...
export const sendChat = async (
  ...
  try {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            chat_title: {
              type: "string",
              description:
                "a suggested name for this conversation, this is the first question in this chat with you, you would be supplied context for follow questions.",
            },
          },
          required: ["answer", "chat_title"],
        },
      };
    }
    ...
    if (json.chat_title) {
      Chats.update({ name: json.chat_title }, { where: { id: chat_id } });
    }
    ...
    return res.status(200).json({ answer: json.answer, chat_id, chat_title: json.chat_title });
  } catch (error) {
    console.log(error);
